...
function Band({
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      [j1, j2].forEach((ref) => {
        const current = ref.current as unknown as Lerped;
        if (!current.lerped)
          current.lerped = new THREE.Vector3().copy(ref.current.translation());
        const clampedDistance = Math.max(
          0.1,
          Math.min(1, current.lerped.distanceTo(ref.current.translation())),
        );
        current.lerped.lerp(
      ...
      curve.points[1].copy((j2.current as unknown as Lerped).lerped!);
      curve.points[2].copy((j1.current as unknown as Lerped).lerped!);
